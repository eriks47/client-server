...
RoboCat::RoboCat()
    : GameObject(), mMaxRotationSpeed(5.f), mMaxLinearSpeed(50.f), mVelocity(Vector3::Zero), mWallRestitution(0.1f),
      mCatRestitution(0.1f), mThrustDir(0.f), mPlayerId(0), mIsShooting(false), mHealth(10)
{
...
{
    // just set the velocity based on the thrust direction -- no thrust will
    // lead to 0 velocity simulating acceleration makes the client prediction a
    // bit more complex
...
void RoboCat::ProcessCollisions()
{
    ...
    // object is not efficient. it would be preferable to use a quad tree or
    // some other structure to minimize the number of collisions that need to be
    // tested.
         ...
         goIt != end; ++goIt)
    {
        ...
        {
            // simple collision test for spheres- are the radii summed less than
            // the distance?
            ...
            {
                // first, tell the other guy there was a collision with a cat,
                ...
                if (target->HandleCollisionWithCat(this))
                {
                    ...
                    // important note- we only move this cat. the other cat can
                    // take care of moving itself
                    ...
                    // got vel with dir between objects to figure out if they're
                    // moving towards each other and if so, the magnitude of the
                    // impulse ( since they're both just balls )

...
void NetworkManager::ReadIncomingPacketsIntoQueue()
{
    ...
    // keep reading until we don't have anything to read ( or we hit a max
    // number that we'll process per frame )
    ...
    while (receivedPackedCount < kMaxPacketsPerFrameCount)
    {
        ...
        else if (readByteCount > 0)
        {
            ...
            {
                // we made it
                // shove the packet into the queue and we'll handle it as soon
                // as we should... we'll pretend it wasn't received until
                // simulated latency from now this doesn't sim jitter, for that
                // we would need to.....
...
NetworkManager::ReceivedPacket::ReceivedPacket(float inReceivedTime, InputMemoryBitStream &ioInputMemoryBitStream,
    ...
    : mReceivedTime(inReceivedTime), mFromAddress(inFromAddress), mPacketBuffer(ioInputMemoryBitStream)
{
}

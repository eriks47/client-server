...
void ReplicationManagerClient::ReadAndDoCreateAction(InputMemoryBitStream &inInputStream, int inNetworkId)
{
    ...
    // we might already have this object- could happen if our ack of the create
    // got dropped so server resends create request ( even though we might have
    // created )
...
void ReplicationManagerClient::ReadAndDoUpdateAction(InputMemoryBitStream &inInputStream, int inNetworkId)
{
    ...
    // gameObject MUST be found, because create was ack'd if we're getting an
...
{
    // if something was destroyed before the create went through, we'll never
    // get it but we might get the destroy request, so be tolerant of being
    // asked to destroy something that wasn't created
    GameObjectPtr gameObject = NetworkManagerClient::sInstance->GetGameObject(inNetworkId);
    if (gameObject)
    {

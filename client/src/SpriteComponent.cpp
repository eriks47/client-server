...
void SpriteComponent::Draw(const SDL_Rect &inViewTransform)
{
    if (mTexture)
    {
        ...
        // Blit the texture
        SDL_RenderCopyEx(GraphicsDriver::sInstance->GetRenderer(), mTexture->GetData(), nullptr, &dstRect,
                         RoboMath::ToDegrees(mGameObject->GetRotation()), nullptr, SDL_FLIP_NONE);
    }
}